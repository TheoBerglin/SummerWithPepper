...
def main():
    ...
    #broker = ALBroker("pythonBroker", "0.0.0.0", 0, args.ip, 9559)
...
class handle_input_module(ALModule):
...
class Robot(object):
    def __init__(self, fname, ip, port, dryrun, rewrite):
        ...
        if not self.dryrun:
            pythonModule = handle_input_module("pythonModule")
            self.tts = ALProxy("ALTextToSpeech", ip, port)
            self.asr = ALProxy("ALSpeechRecognition", ip, port)
            vocabulary = ["hello", "theo"]
            self.asr.setVocabulary(vocabulary, False)
            self.memory = ALProxy("ALMemory")
            self.memory.subscribeToEvent("WordRecognized", "pythonModule", "on_word_recognized")
